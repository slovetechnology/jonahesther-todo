...
export default function TodoForm(props) {
    ...
    const { forms, setForms, singleData } = props
    ...
    const handleSubmission = () => {
        ...
        if (singleData?.title) {
            ...
            // check for any data in local storage that match the data from singleData
            const fromLocalData = JSON.parse(localStorage.getItem('todos'))
            const result = fromLocalData.map((localItem) => {
                if (localItem.title === singleData.title) {
                    return {
                        ...localItem,
                        title: forms.title,
                        price: forms.price,
    ...
    return (
        <div className="w-11/12 mx-auto mt-10">
            ...
            <button
                ...
                className="bg-blue-500 py-3 px-6 w-full rounded-lg text-white capitalize">{singleData?.title ? 'Update Todo' : "add todo"}</button>
        </div>
    )
}